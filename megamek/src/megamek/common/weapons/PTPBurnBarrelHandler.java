...
public class PTPBurnBarrelHandler extends AmmoWeaponHandler {
     ...
     * @param g
     */
    public PTPBurnBarrelHandler(ToHitData toHit, WeaponAttackAction waa, Game g, GameManager m) {
        super(toHit, waa, g, m);
    ...
    protected void handlePartialCoverHit(Entity entityTarget, Vector<Report> vPhaseReport,
        ...
        if (damageableCoverType == LosEffects.DAMAGABLE_COVER_DROPSHIP) {
            ...
            hits = calcHits(vPhaseReport);
            // Burn Barrels do randomly 1-3 damage per-hit to buildings
            Random random = new Random();
            int rDamage = random.nextInt(3)+1;
            int nDamage = rDamage * hits;
    ...
    protected void handleIgnitionDamage(Vector<Report> vPhaseReport, Building bldg, int hits) {
        ...
        if (tn.getValue() != TargetRoll.IMPOSSIBLE) {
            Report.addNewline(vPhaseReport);
            gameManager.tryIgniteHex(target.getPosition(), subjectId, true, true,
                    tn, true, -2, vPhaseReport);
        }
    }
    ...
    protected void handleClearDamage(Vector<Report> vPhaseReport, Building bldg, int nDamage) {
        ...
        Random random = new Random();
        int rDamage = random.nextInt(3)+1;
        nDamage *= rDamage; // Plasma weapons deal double damage to woods.
    ...
    protected void handleBuildingDamage(Vector<Report> vPhaseReport, Building bldg, int nDamage,
        ...
        Random random = new Random();
        int rDamage = random.nextInt(3)+1;
        super.handleBuildingDamage(vPhaseReport, bldg, nDamage * rDamage, coords);
    }
}
